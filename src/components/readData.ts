...
export function readData(a?: objFilter) {
  ...
  onValue(entriesRef, (snapshot) => {
    const table = document.querySelector("div.Data__wraptable") as HTMLElement;
    ...
    snapshot.forEach((element) => {
      if (a) {
        ...
        if (a.id !== "") {
          allBoolean = allBoolean && element.val().entriesId === a.id;
        }
        if (a.state !== "") {
          allBoolean = allBoolean && element.val().state === a.state;
        }
        if (a.date !== "") {
          allBoolean =
            allBoolean &&
